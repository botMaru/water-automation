...
void draw_dashboard_static(){
    ...
    tft.fillRect(0, 128, 128, 32, ST77XX_BLACK); // Clear the bottom area for text
    tft.setTextColor(ST77XX_WHITE);
    tft.setTextSize(1);

    tft.setCursor(5, 128);
    tft.print("Tank: ");
    tft.setCursor(5, 128+10);
    tft.print("Well: ");
}
...
void draw_dashboard_dynamic(){
    ...
    if (current_tank_status != last_tank_status) {
        tft.fillRect(35, 128, 41, 8, ST77XX_BLACK); // Clear the area for text
    ...
    if (current_well_status != last_well_status) {
        tft.fillRect(35, 128+10, 41, 8, ST77XX_BLACK); // Clear the area for text
        tft.setTextColor(current_well_status ? ST77XX_GREEN : ST77XX_RED);
        tft.setCursor(35, 128+10);
...
void draw_water_level() {
    ...
    tft.fillRect((128/2)-(container_size/2), (128/2)-(container_size/2), 
        container_size, (container_size-filled_height), ST7735_WHITE); // Draw the empty part
    tft.fillRect((128/2)-(container_size/2), (128/2)-(container_size/2)+(container_size-filled_height), 
        container_size, filled_height, WATER_COLOR); // Draw the filled part
    tft.setTextColor(ST77XX_BLACK);
    tft.setTextSize(2);
    tft.setFont(&FreeSansBold9pt7b);

    if (water_level_percentage >= 100) {
        tft.setCursor((128/2)-48, (128/2)+5);
    } else {
        tft.setCursor((128/2)-35, (128/2)+5);
    }
    tft.printf("%d%%", water_level_percentage);
    tft.setFont();
}